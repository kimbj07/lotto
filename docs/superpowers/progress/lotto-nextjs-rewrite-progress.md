...
- **#29** **5등 노리기 (`target5`)** — 4th recommendation mode: one 5-game slip whose 30 numbers never overlap, maximising P(≥1 game matches 3+ → 5th prize). Exact enumeration over C(45,6): 11.87% vs 11.36% for 5 independent games (2.38% for one); expected win count is layout-invariant. `recommendTarget5()` (round-robin includes, exclude ≤15), `/api/recommend` now returns `{ games: number[][] }` for every mode (`numbers` kept as a deprecated mirror for single-game modes); target5 records 5 rows under a shared `slip_id` (retries without it, once per instance, if 008 isn't applied); per-mode config centralized in `lib/recommendModes.ts`, `RecommenderClient` slip result + odds note, `ResultsClient` `SlipStats` line, **migration 008** (`slip_id`, `slip_total/slip_graded/slip_hit` on `recommendation_mode_summary`, `refresh_recommendation_summary()` rebuilt, anon EXECUTE revoked). **Also carries the whole-codebase fixes from a 6-lane review**: `lib/latestGameNo.ts` replaces five swallowed-`.single()`-error sites (sync used to re-fetch from draw 1, recommend recorded picks against draw 1); `/api/sync` stops at the first failed draw (no permanent holes), caps 10 draws/run, `maxDuration=60`, 8s upstream timeout, reports RPC errors; `/api/my-numbers` pages past the PostgREST 1000-row cap (was silently truncating); `/api/stats` validation; `middleware.ts` per-IP rate limit on `/api/recommend`; `lib/apiError.ts` + `lib/fetchJson.ts` error contract (no raw DB text to clients, `res.ok` before parse); `Cache-Control` on the three read routes; `StatsClient` stale-response guard; **ESLint configured for the first time** + GitHub Actions CI; unused Geist fonts removed; OG font pinned in `outputFileTracingIncludes`. Tests 111 → 142.
- **#30** **패턴 검증** on `/stats` — `lib/patterns.ts` tests the folk hypotheses (prev bonus repeats, prev numbers repeat, all-time hot/cold, recent hot) on the full history vs the 6/45 base rate (z-score, |z|<2 = 차이 없음), chi-square uniformity (df 44) and split-half persistence; `/api/stats/patterns` reads history in 900-row chunks (PostgREST cap), cached until the cron evicts; `PatternCheckClient` renders table + cards + 결론. Motivation: the owner asked whether such patterns exist — on 1,238 draws none do (all |z| < 1.2, χ² = 29.4).

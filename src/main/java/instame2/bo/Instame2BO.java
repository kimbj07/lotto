...
public class Instame2BO {
	...
	public void registerUser(String code, String me2dayUserId) {
		...
		InstragramUserAccess userAccess = (InstragramUserAccess)JSONObject.toBean(executeRequest(post), InstragramUserAccess.class);
	...
	private void insertInstagramUserInfo(String me2dayUserId, InstragramUserAccess userAccess, String instagramSubscriptionId) {
	...
	public void syncMe2Day(String requestBody) {
		...
		if (userInfo == null) {
			throw new Instame2Exception(Instame2Constants.ERROR + "미등록된 사용자입니다.\n인스타그램 ObjectId : " + objectId + "\n");
		}
	...
	private Me2DayUser acquireMe2dayUserKey(String userId) {
		JSONObject me2dayUser = executeRequest(new PostMethod(Instame2Constants.ME2DAY_GET_PERSON + userId + ".json"));
	...
	private JSONObject executeRequest(HttpMethod method) {
		...
		try {
			...
			json = (JSONObject)JSONSerializer.toJSON(IOUtils.toString(method.getResponseBodyAsStream(), "UTF-8"));
		} catch (Exception e) {
			log.error(e, e);
		} finally {
